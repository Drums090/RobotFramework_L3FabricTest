...
Initiate EVPN Validation
    ...
    \  run keyword and continue on failure  Run Keyword If  '${cPeer}' not in @{target_peers}  Fail EVPN Test Additional Peer  ${cPeer}
    Log  ${established_peers}
    Log  ${target_peers}
    :FOR  ${listObject}  IN  @{target_peers}
    \  run keyword and continue on failure  Run Keyword If  '${listObject}' not in @{established_peers}  Fail EVPN Test Peer List is Incomplete  ${listObject}
Pass EVPN Test
    [Arguments]  ${peer}
    Log  ${peer} is Established
Fail EVPN Test Additional Peer
    [Arguments]  ${peer}
    fail  Unexpected Peering Present: ${peer}
Fail EVPN Test Peer List is Incomplete
    [Arguments]  ${peer}
